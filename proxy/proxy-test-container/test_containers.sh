...
if [ "$TEST_SWARM" = true ]; then
    ./lib/test_swarm_containers.sh "${LOGS_DIR}/${LOG_PREFIX}_test_swarm_results_${LOG_SUFFIX}.log" "$TENANT_ID" $NUM_CONTAINERS
fi

if [ "$TEST_KUBE" = true ]; then
    ./lib/test_kube_pods.sh "${LOGS_DIR}/${LOG_PREFIX}_test_kube_pods_results_${LOG_SUFFIX}.log" "$TENANT_ID" $NUM_PODS
fi
