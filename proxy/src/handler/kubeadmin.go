...
func KubeAdminEndpointHandler(w http.ResponseWriter, r *http.Request) {
	...
	if creds.Status == 200 {
		...
		glog.Info("Will not execute CCSAPI auth")
	} else {
...
func kubeAdminHandler(w http.ResponseWriter, r *http.Request, kubeTarget string,
	...
	case 409:
		glog.Warningf("Namespace %s already exists ", namespace)
	default:
		glog.Errorf("Case default")
	...
	if err != nil {
		glog.Errorf("Error executing kubeSetQuotaRequest: %v", err)
	}
	switch resp.StatusCode {
	case 201:
		glog.Infof("Quota successfully set for amespace %s", namespace)
	case 403:
		glog.Warningf("Quota already set for namespace %s", namespace)
	default:
		glog.Errorf("Case default")
	...
	if err != nil {
		glog.Errorf("Error executing kubeSetLimitsRequest: %v", err)
	}
	switch resp.StatusCode {
	case 201:
		glog.Infof("Limits successfully set for amespace %s", namespace)
	case 409:
		glog.Warningf("Limits already set for namespace %s", namespace)
	default:
		glog.Errorf("Case default")
	...
	if err != nil {
		glog.Errorf("Error executing kubeCreateUserRequest: %v", err)
	}
	//w http.ResponseWriter, r *http.Request, status int
	switch resp.StatusCode {
	case 200:
		glog.Infof("User successfully created for amespace %s", namespace)
		OkHandler(w, r, 200)
	case 201:
		glog.Infof("User successfully created for amespace %s", namespace)
		OkHandler(w, r, 200)
	case 409:
		glog.Warningf("User already set for namespace %s", namespace)
